    ...
    <div class="task">
      <h5>{{title}}</h5>
       ...
       <div class="card card-body border-0 rounded-3">
        <div v-for="task in filterIsCompleted()" :key="task.id">
          <div v-if="filterIsCompleted().length!==0" class="each-task d-flex flex-row align-items-center card card-body border-0 shadow mb-3 rounded-3">
        ...
        <div v-if="filterIsCompleted().length==0">
...
export default {
  ...
  methods:{
    ...
    filterIsCompleted(){
      ...
      console.log('completed?', this.isCompleted, sortedTask.length)
      return sortedTask
    },
    updateTask(task){
